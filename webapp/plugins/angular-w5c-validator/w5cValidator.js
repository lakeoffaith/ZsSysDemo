angular.module("w5c.validator", ["ng"])
    .provider('w5cValidator', [function () {
        var defaultRules = {
                required      : "该选项不能为空",
                maxlength     : "该选项输入值长度不能大于{maxlength}位",
                minlength     : "该选项输入值长度不能小于{minlength}位",
                email         : "输入邮件的格式不正确",
                repeat        : "两次输入不一致",
                pattern       : "该选项输入格式不正确",
                number        : "必须输入数字",
        ...
        var validatorFn = function () {
            ...
            this.defaultShowError = function (elem, errorMessages) {
                ...
                if (!$next || !$next.hasClass("w5c-error")) {
                    $elem.after('<span class="w5c-error">' + errorMessages[0] + '</span>');/*--验证错误提示语--*/
                }
            };
        ...
        validatorFn.prototype = {
            ...
            getErrorMessage : function (validationName, elem) {
                ...
                switch (validationName) {
                    ...
                    case "notZero":
                        if (msgTpl !== null) {
                            return msgTpl.replace("{notZero}", elem.getAttribute("notZero"));
                        }
                        return defaultRules.notZero.replace("{notZero}", elem.getAttribute("notZero"));
                    default :
                    {
                        if (msgTpl !== null) {
                            return msgTpl;
